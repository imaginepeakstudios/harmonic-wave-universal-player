...
> **Status:** Pre-1.0 / engine implementation in progress (Steps 1-10 of 15 complete: registry snapshot, schema interpreter (incl. production wire-shape parsing), MCP client, recipe engine + BehaviorConfig, composition + 5 content renderers + chrome + theme + boot orchestrator with state-machine-driven auto-advance, visualizer canvas + palette extractor + waveform bars + AnalyserNode-backed amplitude provider, 3 lyric overlay variants + LRC parser + generic text-overlay (broadcast-chyron pattern), banner-static + banner-animated scene renderers, **and Step 9: state machine (pure event emitter, iOS-gesture audio-unlock gate, advanceCounter for stale-callback guards) + desktop/mobile audio pipelines (MediaElementSource → Analyser → Gain on desktop; no-op shim on iOS for the coexistence trap) + browser-synthesized music-bed default (mood-driven Web Audio drone — zero external dependencies, the bed always feels scored for THIS moment) + cross-fade transitions on `behavior.transition === 'crossfade'` (visual opacity ramp + audio fadeOut on the old layer-set running concurrently with the new layer-set fading in)**. The engine boots, fetches an HWES response from one of three sources, resolves per-item BehaviorConfig, picks layers (scene → content → overlay → chrome → narration), and mounts the right renderers in z-order. Steps 10-12 add interactions (keyboard/gestures), narration with browser-TTS-default, and the end-of-experience moment. **Platform Phase 1 LIVE in production** (harmonic-wave-api-platform v0.9.76, 2026-04-19) — the data contract this engine builds against is now stable, and the MCP endpoint is reachable cross-origin from any host. See [`docs/SPEC.md`](docs/SPEC.md) for the engineering plan and [the public spec page](https://harmonicwave.ai/hwes/v1) for the consumed JSON shape.
...
bun run test        # 415 tests — should be green
